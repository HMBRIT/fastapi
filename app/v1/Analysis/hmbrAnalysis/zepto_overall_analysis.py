#Sales of Zepto Total for the month
#Sales of HMBR
#Sales of Fixit
#Sales of Ecommerce
#Sales of Rahima

#sales of Zepto Item for the month
#Sales of Section item

#Purchase according to GRN for the month ****
#Payment made ** Change in Payable balance ***
...
def zeptoOverAll():
    ...
    def get_sales(zid,year,month):
        ...
        engine = create_engine('postgresql://postgres:postgres@localhost:5432/da')
        df = pd.read_sql("""SELECT imtrn.xcus,imtrn.xitem, imtrn.xyear, imtrn.xper, imtrn.xdate , imtrn.xqty, imtrn.xdoctype ,imtrn.xdocnum, opddt.xrate , opddt.xlineamt, opdor.xdiscamt, opdor.xtotamt, opdor.xsp
                        FROM imtrn
                        JOIN opddt
    ...
    def get_acc_payable(zid, proj, year, month):
        year_month = str(year) + str(month)
        engine = create_engine('postgresql://postgres:postgres@localhost:5432/da')
        df = pd.read_sql("""SELECT gldetail.xsub,glheader.xyear,glheader.xper,casup.xshort,SUM(gldetail.xprime) as AP
                            FROM glheader
                            JOIN gldetail
                            ON glheader.xvoucher = gldetail.xvoucher
                            JOIN casup
                            ...
                            AND gldetail.xvoucher NOT LIKE '%s'
                            AND CONCAT(glheader.xyear,glheader.xper) <= '%s'
                            GROUP BY gldetail.xsub,glheader.xyear,glheader.xper,casup.xshort"""%(zid,zid,zid,proj,'OB--%%',year_month),con=engine)
        return df
    ...
    df_sales_g_z = df_sales_z.groupby(['xcus','xyear','xper','xsp','businessId'])['xlineamt'].sum().reset_index().round(2)
    df_return_g_z = df_return_z.groupby(['xcus','xyear','xper','xsp'])['totamt'].sum().reset_index().round(2)

    # #final for all
    df_zepto_g_z = df_cus_z.merge(df_sales_g_z[['xcus','xyear','xper','xsp','businessId','xlineamt']],on=['xcus'],how='left').merge(df_return_g_z[['xcus','xyear','xper','xsp','totamt']],on=['xcus','xyear','xper','xsp'],how='left').fillna(0)
    ...
    df_zp = get_acc_payable(zid_zepto, proj_zepto, end_year, end_month)
    df_zp['xyear'] = df_zp['xyear'].astype(np.int64)
    df_zp['xper'] = df_zp['xper'].astype(np.int64)
    df_zp['time_line'] = df_zp['xyear'].astype(str)+'/'+df_zp['xper'].astype(str)
    df_zp = df_zp[df_zp['xyear']==end_year]
    df_zp = pd.pivot_table(df_zp,values='ap', index=['xsub','xshort'],columns=['time_line'],aggfunc=np.sum).reset_index().fillna(0)



    df_sales_i_z = df_sales_z.groupby(['xitem','xyear','xper'])['xlineamt'].sum().reset_index().round(2)
    df_return_i_z = df_return_z.groupby(['xitem','xyear','xper'])['totamt'].sum().reset_index().round(2)
    # df_zepto_g_z = df_cus_z.merge(df_sales_g_z[['xcus','xyear','xper','xsp','businessId','xlineamt']],on=['xcus'],how='left').merge(df_return_g_z[['xcus','xyear','xper','xsp','totamt']],on=['xcus','xyear','xper','xsp'],how='left').fillna(0)

    df_caitem = get_item(zid_zepto)
    df_item = df_caitem.merge(df_sales_i_z[['xitem','xyear','xper','xlineamt']],on='xitem',how='left').merge(df_return_i_z[['xitem','xyear','xper','totamt']],on=['xitem','xyear','xper'],how='left').fillna(0)
    df_item['Sales'] = df_item['xlineamt'] - df_item['totamt']

    df_item = df_item.drop(columns=['xlineamt','totamt'])
    ...
    msg['Subject'] = "Zepto Overall Accounts Detail"
    msg['From'] = me
    msg['To'] = ", ".join(you)
