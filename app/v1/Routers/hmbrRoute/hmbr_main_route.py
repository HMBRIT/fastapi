...
@router.get("/api/v1/transport", tags=["transport"])
async def transport ():
    x = transportAnalysis ()
    return {"data": x}


@router.get("/api/v1/salesreport", tags=["salesreport"])
async def salesreport ():
    salesReportMain()
    return {"data": "success"}

@router.post("/api/v1/zeptooverall", tags=["zepto overall reports"] , status_code=status.HTTP_200_OK)
